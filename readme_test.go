...
func TestReadmeToBeString(t *testing.T) {
	...
	expect.Value(t, "error", l.Messages[0]).ToBe("readme_test.go:12 TestReadmeToBeString\nexpected the house to be 'small' but it is 'big'")
}

func TestReadmeToToBeArray(t *testing.T) {
	l := &test.Logger{}
	expect.Value(l, "array", []int{3, 1}).ToBe([]int{1, 3})
	expect.Value(t, "error", l.Messages[0]).ToBe(`readme_test.go:18 TestReadmeToToBeArray
